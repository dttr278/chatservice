...
var token, currentId, preClick, userName, userAvatar, userId, host;
...
$(document).ready(function () {
 ...
 //   alert(userName + ">>" + userId + ">>" + userAvatar);
...
function init() {
    ...
    $("#name").text(userName);
    if (userAvatar != '')
        $("#avatar").attr('src', host + 'file/download/' + userAvatar);
    $(".chats").show();
...
function getContacts() {
    ...
    $.get(host + "contacts/9999", function (data, status) {
        if (data.result != 0 && data.result != null) {
            // a.remove();
...
function getMessages(chatId) {
    if (chatId != null) {
        $.get(host + "messages/" + chatId + "/9999", function (data, status) {
            if (data.result != 0 && data.result != null) {
...
function getGroups() {
    ...
    $.get(host + "groups/9999", function (data, status) {
        a.remove();
        if (data.result != 0 && data.result != null) {
